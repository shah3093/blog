...
<div class="row">
    ...
    @include('backend.partials.errors')
            ...
            <form id="addFrm" action="{{route('backend.quizzes.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select id="post" name="post" class="form-control select2 required">
                                    ...
                                    @foreach($posts as $post)
                                        <option {{$post->id == old('post') ? 'selected':''}} value="{{$post->id}}">{{$post->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
